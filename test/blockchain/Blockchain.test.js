...
describe('Blockchain', () => {
  ...
  it('should generate a new transaction to process when createTransactions is called', () => {
    const chain = new Blockchain()

    chain.createTransaction({ from: 'test1', to: 'test2', amount: 24 })
  ...
  it('should not mine a new block when there are no transactions in the queue', () => {
    ...
    expect(chain.blockchain.length).equal(1)
  })
